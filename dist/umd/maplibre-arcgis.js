/* @esri/maplibre-arcgis - v1.3.1 - Mon Aug 10 2026 15:21:57 GMT-0700 (Pacific Daylight Time)
...
var AttributionControl = class _AttributionControl extends import_maplibre_gl.AttributionControl {
  /**
...
function internalRequest(url, requestOptions) {
  ...
  if (typeof options.authentication === "string") {
    ...
    if (!options.authentication.startsWith("AAPK") && !options.authentication.startsWith("AAPT") && !options.authentication.startsWith("AATK") && // doesn't look like an API Key
...
// node_modules/@esri/arcgis-rest-request/dist/esm/utils/getOauthStateId.js
...
// node_modules/@esri/arcgis-rest-request/dist/esm/utils/encodeOauthState.js
function encodeOauthState(stateId, state, originalUrl) {
  if (typeof state === "string" || !state) {
    return JSON.stringify({ id: stateId, originalUrl });
  }
  return JSON.stringify(Object.assign({ id: stateId }, state));
}

// node_modules/@esri/arcgis-rest-request/dist/esm/ArcGISIdentityManager.js
function isCredential(credential) {
  return typeof credential.userId === "string" || /* istanbul ignore next -- @preserve */
...
var ArcGISIdentityManager = class _ArcGISIdentityManager extends AuthenticationManagerBase {
  ...
  static completeOAuth2(options, win) {
    ...
    function createManager(oauthInfo, state2) {
      win.localStorage.removeItem(stateStorageKey);
      if (popup && win.opener) {
        win.opener.dispatchEvent(new CustomEvent(`arcgis-rest-js-popup-auth-${clientId}`, {
          detail: Object.assign(Object.assign({}, oauthInfo), { state: state2 })
        }));
        win.close();
        return;
      }
      if (state2.originalUrl) {
        win.history.replaceState(win.history.state, "", state2.originalUrl);
      }
    ...
    if (pkce && params.code) {
      ...
      return fetchToken(tokenEndpoint, {
        ...
        return createManager(Object.assign(Object.assign({}, tokenResponse), state), state);
      }).catch((e) => {
        return reportError(e.originalMessage, e.code, state.originalUrl);
      });
    }
    if (!pkce && params.access_token) {
      return Promise.resolve(createManager(Object.assign({ token: params.access_token, expires: new Date(Date.now() + parseInt(params.expires_in, 10) * 1e3), ssl: params.ssl === "true", username: params.username }, state), state));
    }
...
var wrapAccessToken = async (token, portalUrl) => {
...
var BasemapStyle = class _BasemapStyle {
  ...
  applyTo(map, maplibreStyleOptions) {
    ...
    if (this.session) {
      this.session.on("BasemapSessionRefreshed", (sessionData) => {
        ...
        this._updateToken(oldToken, newToken, map);
      });
    }
    return this._map;
  }
  ...
  async loadStyle() {
    ...
    if (this._isItemId && this.session) {
      authentication = ApiKeyManager.fromKey(this.session.parentToken);
    ...
    if (style.sprite) {
      let spriteToken;
      if (this.session) spriteToken = this.session.parentToken;
  ...
  _updatePreferences(preferences) {
    ...
    if (this._isItemId) {
      warn2("Preferences such as 'language', 'places', and 'worldview' are not supported with custom basemaps IDs. These parameters will be ignored.");
      return;
    }
  ...
  _updateToken(fromToken, toToken, map) {
    ...
    for (const tileManagerId of Object.keys(this._map.style.tileManagers)) {
      ...
      newStyle.sources[tileManagerId].tiles = newTiles;
    }
...
function getItem(id, requestOptions) {
  ...
  return request(url, options).then(async (item) => {
    ...
    let token;
    if ((requestOptions === null || requestOptions === void 0 ? void 0 : requestOptions.authentication) && typeof requestOptions.authentication !== "string" && item.access !== "public") {
...
// node_modules/@esri/arcgis-rest-feature-service/dist/esm/pbf-parser/PbfFeatureCollectionV2.js
function readFeatureCollectionPBuffer(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferField, { version: "", queryResult: void 0 }, end);
}
...
var FeatureCollectionPBufferFieldType = {
...
var FeatureCollectionPBufferSQLType = {
  sqlTypeBigInt: 0,
  sqlTypeBinary: 1,
  sqlTypeBit: 2,
  sqlTypeChar: 3,
...
function readFeatureCollectionPBufferGeometryField(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferGeometryFieldField, { field: void 0, geometryType: 0 }, end);
}
...
function readFeatureCollectionPBufferEnvelope(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferEnvelopeField, { XMin: 0, YMin: 0, XMax: 0, YMax: 0, SpatialReference: void 0 }, end);
}
...
function readFeatureCollectionPBufferesriShapeBuffer(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferesriShapeBufferField, { bytes: void 0 }, end);
}
...
function readFeatureCollectionPBufferFeature(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferFeatureField, {
    attributes: [],
    geometry: void 0,
    compressed_geometry: void 0,
    shapeBuffer: void 0,
    curveGeometry: void 0,
    centroid: void 0,
    aggregateGeometries: [],
    envelope: void 0
  }, end);
...
function readFeatureCollectionPBufferTransform(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferTransformField, { quantizeOriginPostion: 0, scale: void 0, translate: void 0 }, end);
}
...
function readFeatureCollectionPBufferFeatureResult(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferFeatureResultField, {
    objectIdFieldName: "",
    uniqueIdField: void 0,
    globalIdFieldName: "",
    geohashFieldName: "",
    geometryProperties: void 0,
    serverGens: void 0,
    geometryType: 0,
...
function readFeatureCollectionPBufferObjectIdsResult(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferObjectIdsResultField, { objectIdFieldName: "", serverGens: void 0, objectIds: [] }, end);
}
...
function readFeatureCollectionPBufferExtentCountResult(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferExtentCountResultField, { extent: void 0, count: 0 }, end);
}
...
function readFeatureCollectionPBufferQueryResult(pbf, end) {
  return pbf.readFields(_readFeatureCollectionPBufferQueryResultField, {
    featureResult: void 0,
    Results: void 0,
    countResult: void 0,
    idsResult: void 0,
    extentCountResult: void 0
  }, end);
}
...
// node_modules/@esri/arcgis-rest-feature-service/dist/esm/pbf-parser/geoJSONPbfParser.js
function pbfToGeoJSON(arrayBuffer) {
...
function decode(featureCollectionBuffer) {
  ...
  try {
    ...
    throw new Error(`Could not parse arcgis-pbf buffer: ${error2}`);
  }
  ...
  if (fields.every((field) => field.name !== objectIdField)) {
    throw new ArcGISRequestError(`objectIdField '${objectIdField}' was not found.`, 400);
  }
...
function collectAttributes(fields, featureAttributes) {
  ...
  for (let i = 0; i < fields.length; i++) {
    const f = fields[i];
    if (featureAttributes[i][featureAttributes[i].value_type] !== void 0)
...
function transformTuple(coords, transform) {
  let x = coords[0];
  let y = coords[1];
  let z = coords[2] ? coords[2] : void 0;
  if (transform.scale) {
...
// node_modules/@esri/arcgis-rest-feature-service/dist/esm/pbf-parser/arcGISPbfParser.js
function pbfToArcGIS(featureCollectionBuffer) {
  const decodedObject = decode2(featureCollectionBuffer);
  const featureResult = decodedObject.queryResult.featureResult;
  ...
  const attributeFields = featureResult.fields.map((field) => Object.assign(Object.assign({}, field), { keyName: getKeyName2(field) }));
  const geometryParser = getGeometryParser2(geometryType);
  out.features = featureResult.features.map((f) => ({
    attributes: collectAttributes2(attributeFields, f.attributes),
    geometry: f.geometry && geometryParser(f, transform) || null
  }));
  const queryFeaturesResponse = normalizeFeatureResponse(out);
  return queryFeaturesResponse;
}
function decode2(featureCollectionBuffer) {
  let decodedObject;
  try {
    decodedObject = readFeatureCollectionPBuffer(new Pbf(featureCollectionBuffer));
  } catch (error2) {
...
function decodeField(field, fieldTypeMap, sqlTypeMap) {
  const optionalProps = [
    ["alias", (f) => f.alias],
    ["sqlType", (f) => sqlTypeMap[f.sqlType]],
    ["domain", (f) => f.domain === "" ? null : JSON.parse(f.domain)],
    ["length", (f) => f.length === 0 ? void 0 : f.length],
...
function collectAttributes2(fields, featureAttributes) {
  const out = {};
  for (let i = 0; i < fields.length; i++) {
    const f = fields[i];
    if (featureAttributes[i][featureAttributes[i].value_type] !== void 0)
...
function normalizeFeatureResponse(featureResult) {
  ...
  Object.keys(featureResult).forEach((key) => {
    if (!excludeKeys.includes(key) && featureResult[key] !== void 0) {
      out[key] = featureResult[key];
    }
  });
  return out;
}
function getGeometryParser2(featureType) {
  switch (featureType) {
    case 3:
...
function createPoint2(f, transform) {
  const p = {
    type: "Point",
    coordinates: transformTuple2(f.geometry.coords, transform)
  };
  const ret = {
    x: p.coordinates[0],
    y: p.coordinates[1]
...
function createLine2(f, transform) {
  let l = null;
  const lengths = f.geometry.lengths.length;
  if (lengths === 1) {
    l = {
      type: "LineString",
      coordinates: createLinearRing2(f.geometry.coords, transform, 0, f.geometry.lengths[0] * 2)
    };
    return {
      paths: [l.coordinates]
    };
    ...
    for (let index = 0; index < lengths; index++) {
      const stopPoint = startPoint + f.geometry.lengths[index] * 2;
      const line = createLinearRing2(f.geometry.coords, transform, startPoint, stopPoint);
      l.coordinates.push(line);
      startPoint = stopPoint;
    }
    return {
...
function createPolygon2(f, transform) {
  const lengths = f.geometry.lengths.length;
  const p = {
    type: "Polygon",
    coordinates: []
  ...
  if (lengths === 1) {
    p.coordinates.push(createLinearRing2(f.geometry.coords, transform, 0, f.geometry.lengths[0] * 2));
  } else {
    p.type = "MultiPolygon";
    let startPoint = 0;
    for (let index = 0; index < lengths; index++) {
      const stopPoint = startPoint + f.geometry.lengths[index] * 2;
      const ring = createLinearRing2(f.geometry.coords, transform, startPoint, stopPoint);
      if (ringIsClockwise2(ring)) {
        p.coordinates.push([ring]);
      } else if (p.coordinates.length > 0) {
        p.coordinates[p.coordinates.length - 1].push(ring);
      }
...
function ringIsClockwise2(ringToTest) {
  let total = 0;
  let i = 0;
  const rLength = ringToTest.length;
  let pt1 = ringToTest[i];
...
function createLinearRing2(arr, transform, startPoint, stopPoint) {
  const out = [];
  if (arr.length === 0)
    return out;
  const initialX = arr[startPoint];
  const initialY = arr[startPoint + 1];
  out.push(transformTuple2([initialX, initialY], transform));
  let prevX = initialX;
  let prevY = initialY;
  for (let i = startPoint + 2; i < stopPoint; i = i + 2) {
    const x = difference2(prevX, arr[i]);
    const y = difference2(prevY, arr[i + 1]);
    const transformed = transformTuple2([x, y], transform);
    out.push(transformed);
    prevX = x;
    prevY = y;
  }
...
function transformTuple2(coords, transform) {
  let x = coords[0];
  let y = coords[1];
  let z = coords[2] ? coords[2] : void 0;
  if (transform.scale) {
...
function queryPbfAsGeoJSONOrArcGIS(url, queryOptions) {
  if (queryOptions.params.f === "pbf-as-geojson" && queryOptions.params.outSR && queryOptions.params.outSR !== "4326") {
    throw new ArcGISRequestError("Unsupported outSR for GeoJSON requests.", 422, null, url, queryOptions);
  }
  ...
  return request(`${cleanUrl(url)}/query`, customOptions).then(async (response) => {
    var _a;
    if ((_a = response.headers.get("content-type")) === null || _a === void 0 ? void 0 : _a.includes("application/json")) {
      ...
      if ((err === null || err === void 0 ? void 0 : err.code) === 498 || (err === null || err === void 0 ? void 0 : err.code) === 499) {
        throw new ArcGISAuthError(err.message, err.code, response, url, customOptions);
      }
      throw new ArcGISRequestError(err.message, err.code, response, url, customOptions);
    }
    try {
      ...
      if (error2 instanceof ArcGISRequestError) {
        error2.response = response;
        error2.url = url;
        error2.options = customOptions;
        throw error2;
      } else {
        throw new ArcGISRequestError("Unable to decode pbf response.", 500, response, url, customOptions);
      }
    }
  });
}
function queryFeatures(requestOptions) {
  ...
  if (((_a = queryOptions.params) === null || _a === void 0 ? void 0 : _a.f) === "pbf-as-geojson" || ((_b = queryOptions.params) === null || _b === void 0 ? void 0 : _b.f) === "pbf-as-arcgis") {
...
async function queryAllFeatures(requestOptions) {
  ...
  const userRecordCount = requestOptions.resultRecordCount || ((_a = requestOptions.params) === null || _a === void 0 ? void 0 : _a.resultRecordCount);
  if (userRecordCount && userRecordCount >= 1e5) {
    throw new ArcGISRequestError("resultRecordCount must be less than 100,000.", 400, null, requestOptions.url, requestOptions);
  }
  ...
  if (userRecordCount) {
    ...
    recordCountToUse = pageSizeResponse.maxRecordCount || 2e3;
  }
  while (hasMore) {
    ...
    let response;
    if (((_b = queryOptions.params) === null || _b === void 0 ? void 0 : _b.f) === "pbf-as-geojson" || ((_c = queryOptions.params) === null || _c === void 0 ? void 0 : _c.f) === "pbf-as-arcgis") {
      response = await queryPbfAsGeoJSONOrArcGIS(requestOptions.url, queryOptions);
    } else {
      response = await request(`${cleanUrl(requestOptions.url)}/query`, queryOptions);
    }
    ...
    const exceededTransferLimit = (
      ...
      response.exceededTransferLimit || // GeoJSON | pbf-as-geojson: exceededTransferLimit is on properties in the response object
...
var HostedLayer = class _HostedLayer {
   ...
   * @returns this
   */
  addSourcesAndLayersTo(map, options) {
   ...
   * @returns this
   */
  addSourcesTo(map, transformSources) {
   ...
   * @returns
   */
  addLayersTo(map, transformLayers) {
...
// src/FeatureLayerSourceManager.ts
var import_maplibre_gl2 = require("maplibre-gl");

// node_modules/@mapbox/tilebelt/dist/esm/index.js
var d2r = Math.PI / 180;
var r2d = 180 / Math.PI;
...
// src/FeatureLayerSourceManager.ts
var FeatureLayerSourceManager = class {
  constructor(id, layerUrl, layerDefinition, options) {
    this.map = void 0;
    this.maplibreSource = void 0;
    this._maxExtent = [-Infinity, Infinity, -Infinity, Infinity];
    this._tileIndices = /* @__PURE__ */ new Map();
    this._featureIndices = /* @__PURE__ */ new Map();
    this._featureCollections = /* @__PURE__ */ new Map();
    ...
    this._snapshotResultRecordCount = Math.min((this.layerDefinition.maxRecordCount ?? 2e3) * this._maxRecordCountFactor, 8e3);
    this._onDemandResultRecordCount = Math.min((this.layerDefinition.tileMaxRecordCount ?? 2e3) * this._maxRecordCountFactor, 8e3);
    this._options = {
  ...
  async _snapshotLoad() {
    ...
    await this._pendingSnapshot;
    return;
  }
  async _attemptSnapshotLoad() {
    if (!this.layerDefinition?.geometryType) {
      throw new Error("Layer definition with geometry type undefined.");
    }
    const geometryLimit = esriGeometryInfo[this.layerDefinition.geometryType].limit;
    const requestParams = {
      url: this.layerUrl,
 ...
 To enable on-demand loading, pass the MapLibre map in the constructor or use a method such as layer.addSourceTo(map). If you are already doing this, you can ignore this message.`);
  ...
  async load() {
    ...
    if (defaultOrSnapshot) {
      const snapshotSucceeded = await (this._pendingSnapshot !== void 0 ? this._pendingSnapshot : this._attemptSnapshotLoad());
  ...
  // =====================
  _bindLoadFeaturesToMoveEndEvent() {
    this.map.on("moveend", () => {
      void this._loadFeaturesOnDemand();
    });
  }
  ...
  _doesTileOverlapBounds(tile, bounds) {
    const tileBBox = tile.length === 4 ? tile : tileToBBOX(tile);
  ...
  _addTileFeaturesToFeatureCollection(tileFc, featureIdIndex, fc) {
  ...
  async _getTile(tile, tolerance) {
    ...
    const res = await queryAllFeatures(queryParams);
    return res;
  }
  // Only 4326 and 3857 are currently handled for service extent.
  _setMaxExtentFromLayerExtent(layerExtent) {
    if (layerExtent.spatialReference?.wkid === 4326) {
      ...
      const sw = new import_maplibre_gl2.MercatorCoordinate(layerExtent.xmin, layerExtent.ymin).toLngLat();
      const ne = new import_maplibre_gl2.MercatorCoordinate(layerExtent.xmax, layerExtent.ymax).toLngLat();
      const extent = new import_maplibre_gl2.LngLatBounds(sw, ne);
   ...
   */
  async _checkIfExceedsLimit(params, geometryLimit) {
    const exceedsLimitParams = {
      ...params,
      outStatistics: [
        {
          onStatisticField: null,
          // Required by REST JS but not used
    ...
    const exceedsLimitResponse = await queryFeatures(exceedsLimitParams);
    return exceedsLimitResponse.features[0].attributes.exceedslimit === 1;
  }
  _updateSourceData(fc, map) {
    if (map) {
  ...
  _isExtentVisible(mapBounds) {
    return this._maxExtent[0] === -Infinity || this._doesTileOverlapBounds(this._maxExtent, mapBounds);
  }
  _getZoomLevel(map) {
    return this._options.useStaticZoomLevel ? this._onDemandSettings.staticZoomLevel : Math.round(map.getZoom());
  }
  _findTilesToRequestAtZoomLevel(mapBounds, zoomLevel) {
  ...
  _filterRequestedTiles(tilesToRequest, tileIndex) {
  ...
  _calculateTolerance(zoomLevel) {
    return 360 / 2 ** (zoomLevel + 1) / 1e3;
  }
};

// src/FeatureLayer.ts
var esriGeometryInfo = {
  esriGeometryPoint: {
    type: "circle",
    limit: {
      maxPointCount: 2e5,
...
var isSupportedServiceType = (serviceType) => {
...
var FeatureLayer = class _FeatureLayer extends HostedLayer {
  ...
  async _initializeLayer(layerUrl) {
    ...
    const options = {
      queryOptions: this.query ?? void 0,
      authentication: this._authentication,
      loadingMode: this.loadingMode,
      map: this._map ?? void 0,
      callback: (features) => {
        this._updateData(sourceId, features);
      }
    };
    ...
    if (this.loadingMode === "ondemand" && !this._map) {
      warn2("On-demand loading mode is enabled. This layer requires access to the map, either by passing via the constructor or by using a method like addSourcesTo(map). If you are already doing this, you can ignore this message.");
    }
    ...
    switch (dataSource) {
      case "FeatureService": {
        ...
        for (let i = 0; i < serviceInfo.layers.length && i < 10; i++) {
          ...
          await this._initializeLayer(`${cleanUrl2(this._serviceInfo.serviceUrl)}${i}/`);
        }
        break;
      }
      case "FeatureLayer": {
...
var VectorTileLayer = class _VectorTileLayer extends HostedLayer {
  ...
  async initialize() {
    ...
    if (this._initOptions.itemId) {
      ...
      this._serviceInfo = {
        serviceUrl: cleanUrl2(this._initOptions.url)
      };
    } else {
...
var package_default = {
  ...
  scripts: {
    ...
    "start-dev": "node build.js dev watch",
    test: "vitest",
    "test:live": "vitest run ./test/**/*.live.test.{ts,js}",
    format: "eslint . --fix",
    docs: "typedoc"
  },
  license: "Apache-2.0",
  contributors: [
    ...
    {
      name: "Dixon Yant",
      email: "dyant@esri.com",
      url: "https://github.com/dixonyant"
    }
  ],
  type: "module",
  peerDependencies: {
    "maplibre-gl": "~5.24.0 || ~6.3.0"
  },
  dependencies: {
...
@esri/arcgis-rest-request/dist/esm/requestConfig.js:
  (* istanbul ignore file -- @preserve *)

@esri/arcgis-rest-request/dist/esm/request.js:
...
@esri/arcgis-rest-request/dist/esm/ApplicationCredentialsManager.js:
@esri/arcgis-rest-request/dist/esm/ApiKeyManager.js:
@esri/arcgis-rest-basemap-sessions/dist/esm/BaseSession.js:
  (* istanbul ignore next -- @preserve *)

@esri/arcgis-rest-request/dist/esm/utils/base-64-url.js:
  (* istanbul ignore next -- @preserve: must pass in a mockwindow for tests so we can't cover the other branch *)

@esri/arcgis-rest-request/dist/esm/utils/generate-code-challenge.js:
@esri/arcgis-rest-request/dist/esm/utils/generate-random-string.js:
  (* istanbul ignore next -- @preserve : must pass in a mockwindow for tests so we can't cover the other branch *)

@esri/arcgis-rest-request/dist/esm/ArcGISIdentityManager.js:
  (* istanbul ignore next -- @preserve : must pass in a mockwindow for tests so we can't cover the other branch *)
...
@esri/arcgis-rest-feature-service/dist/esm/pbf-parser/PbfFeatureCollectionV2.js:
  (* istanbul ignore file --@preserve *)

@esri/arcgis-rest-feature-service/dist/esm/pbf-parser/geoJSONPbfParser.js:
  (* istanbul ignore next --@preserve *)
...
@esri/arcgis-rest-feature-service/dist/esm/query.js:
  (* istanbul ignore else --@preserve *)
*/
